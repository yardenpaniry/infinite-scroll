...
const ImageDetails = ({ route, navigation }) => {
  ...
  const backButton = () => {
    if (location.state) {
      navigate(`/infinite-scroll`, {
        state: {
          pagePosition: location.state.pagePosition,
  ...
  useEffect(() => {
    ...
    } else setDescriptionImage(image.description);
  });

  return (
    <div className="detailsPage">
      <div className="backBtnDiv">
        <button type="button" className="btn backButton" onClick={backButton}>
          &#8249; Back
        </button>
      </div>
